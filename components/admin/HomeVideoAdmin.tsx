...
export default function HomeVideoAdmin() {
  ...
  function selectFile(next: File | null) {
    ...
    if (next.size > MAX_VIDEO_BYTES) {
      setFile(null);
      return setMessage("Ukuran video maksimal 50 MB. File 40 MB masih dapat digunakan.");
    }
    setMessage("");
    setFile(next);
  }
  ...
  async function saveText(event: FormEvent) {
    ...
    if (poster && !safeUrl(poster)) return setMessage("URL thumbnail harus menggunakan HTTPS yang valid.");
    await run(async () => {
    ...
    await run(async () => {
      ...
      try {
        await upsertSettings({
          ...
          home_video_title: (values.home_video_title || "THE STORY BEHIND SKYGOAT").trim(),
          home_video_description: (values.home_video_description || "Kenali perjalanan SKYGOAT lebih dekat melalui video.").trim(),
      ...
      if (!keepPrevious && oldPath && oldPath !== path) {
        ...
        if (removeError) console.warn("Video lama tidak dapat dihapus dari storage", removeError.message);
      }
      ...
      await load();
    }, keepPrevious ? "Video baru aktif. Video sebelumnya disimpan di arsip." : "Video baru aktif. Video sebelumnya diganti.");
  }
  ...
  async function restore(item: ArchiveItem) {
    ...
    await run(async () => {
      ...
      let nextArchive = parseArchive(saved.home_video_archive).filter((entry) => entry.url !== item.url);
  ...
  async function removeArchive(item: ArchiveItem) {
    ...
    await run(async () => {
      ...
      const nextArchive = parseArchive(saved.home_video_archive).filter((entry) => entry.url !== item.url);
  ...
  return (
    <section className="adminPanel homeVideoAdmin">
      <div className="homeVideoAdminHead">
        <div>
          <span className="adminEyebrow">HOMEPAGE</span>
          <h2>Video Tentang SKYGOAT</h2>
          <p>Video tampil sebagai banner dengan tombol Play, setelah strip informasi dan sebelum bagian Tentang SKYGOAT.</p>
        </div>
      ...
      {!loading && !error && <>
        <div
          ...
          aria-label="Upload video dengan drag and drop atau pilih file"
        >
          <input ref={fileInput} type="file" accept="video/mp4,video/webm,video/ogg,.mp4,.webm,.ogg" onChange={(event) => selectFile(event.target.files?.[0] ?? null)} hidden />
          <div className="videoDropIcon">＋</div>
          <strong>{file ? file.name : "Drag & drop video di sini"}</strong>
          <span>{file ? `${bytes(file.size)} · siap di-upload` : "atau klik untuk pilih MP4 / WebM / OGG"}</span>
          <small>Maksimal 50 MB. File video kamu yang sekitar 40 MB bisa dicoba langsung.</small>
        </div>

        <label className="keepOldVideo">
          <input type="checkbox" checked={keepPrevious} onChange={(event) => setKeepPrevious(event.target.checked)} />
          <span><strong>Simpan video lama ke arsip</strong><small>Jika tidak dicentang, video lama di Supabase Storage akan dihapus setelah video baru berhasil disimpan.</small></span>
        </label>

        <button type="button" className="videoUploadBtn" disabled={busy || !file} onClick={uploadVideo}>{busy ? "Mengupload..." : "Upload & jadikan video aktif"}</button>
        <p className="adminHelpText">Jangan tutup halaman saat upload berlangsung. Jika video lama berasal dari URL eksternal, CMS hanya mengganti referensinya dan tidak dapat menghapus file di layanan eksternal.</p>

        <form className="adminForm homeVideoMeta" onSubmit={saveText}>
          <fieldset disabled={busy}>
            <label htmlFor="home-video-title">Judul banner</label>
            <input id="home-video-title" maxLength={120} value={values.home_video_title || "THE STORY BEHIND SKYGOAT"} onChange={(event) => setDraft({ ...draft, home_video_title: event.target.value })} />
            <label htmlFor="home-video-description">Deskripsi singkat</label>
            <textarea id="home-video-description" rows={3} maxLength={300} value={values.home_video_description || "Kenali perjalanan SKYGOAT lebih dekat melalui video."} onChange={(event) => setDraft({ ...draft, home_video_description: event.target.value })} />
            <label htmlFor="home-video-poster">URL thumbnail / poster (opsional)</label>
            <input id="home-video-poster" type="url" placeholder="https://..." value={values.home_video_poster_url || ""} onChange={(event) => setDraft({ ...draft, home_video_poster_url: event.target.value })} />
            <button type="submit">{busy ? "Menyimpan..." : "Simpan teks & thumbnail"}</button>
          </fieldset>
        </form>

        <div className="currentVideoInfo">
          <span>VIDEO AKTIF</span>
          <strong>{saved.home_video_file_name || (currentUrl ? "Video eksternal" : "Belum ada video")}</strong>
          {saved.home_video_file_size && <small>{bytes(Number(saved.home_video_file_size))}</small>}
        </div>

        <div className="videoArchive">
          <div className="videoArchiveHead"><h3>Arsip video lama</h3><span>{archive.length} tersimpan</span></div>
          {!archive.length ? <p className="adminHelpText">Belum ada video lama yang disimpan. Centang “Simpan video lama ke arsip” sebelum upload video pengganti.</p> : archive.map((item) => (
